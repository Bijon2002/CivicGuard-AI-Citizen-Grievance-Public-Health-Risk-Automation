...
class Department(Base):
    ...
    issue_types: Mapped[list[str]] = mapped_column(JSON, nullable=False, default=list)
...
class Report(Base):
    ...
    is_duplicate_of: Mapped[str | None] = mapped_column(String(36), ForeignKey("reports.id"), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
